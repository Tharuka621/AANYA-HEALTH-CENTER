...
const PharmacistDashboard: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const [invRes, prescRes, invsRes] = await Promise.all([
        axiosInstance.get('/pharmacist/inventory'),
        axiosInstance.get('/pharmacist/prescriptions/pending'),
        axiosInstance.get('/pharmacist/invoices')
      ]);
      setInventoryBatches(invRes.data);
  ...
  const handleDispense = (prescriptionId: string) => {
    ...
    if (prescription && prescription.status === 'ACTIVE') {
      ...
      prescription.medicines.forEach(med => {
        const medicineId = med.medicine_id || inventoryBatches.find(
          b => String(b.medicine_name) === String(med.name)
        )?.medicine_id;

        const requiredUnit = med.unit || inventoryBatches.find(b => String(b.medicine_id) === String(medicineId))?.dosage || '';
  ...
  const handleConfirmDispense = async () => {
    ...
    try {
      ...
      const res = await axiosInstance.post(`/pharmacist/prescriptions/${selectedPrescription.id}/dispense`, {
        deductionPlans
      });

      showSuccess(res.data.message);

      // The backend returns the generated invoice, push to local state to show the receipt
      if (res.data.invoice) {
      ...
      // Refresh pending prescriptions & inventory
      fetchData();

    } catch (err: any) {
      showError(err.response?.data?.message || 'Failed to dispense prescription');
    } finally {
  ...
  const handleRecordPayment = async () => {
    ...
    try {
      const res = await axiosInstance.post(`/pharmacist/invoices/${currentInvoice.id}/pay`, {
                            ...
                            <TableRow key={idx} sx={{ bgcolor: hasError ? 'error.light' : 'inherit' }}>
                              <TableCell>
                                ...
                                <Typography variant="caption" color="text.secondary" display="block">
                                  Instructions: {selectedPrescription.medicines[idx]?.dosage}
                                </Typography>
                              </TableCell>
                              <TableCell align="center">
                                <Chip
